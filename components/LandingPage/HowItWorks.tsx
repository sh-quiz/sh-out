            ...
            <div className="sticky top-0 flex h-screen items-center overflow-hidden">
                <motion.div style={{ x }} className="flex gap-4 md:gap-12 px-4 md:px-24">
                    {steps.map((step) => (
                        <div
                            key={step.id}
                            className="group relative h-[50vh] md:h-[60vh] w-[85vw] md:w-[40vw] min-w-[85vw] md:min-w-[500px] bg-[#1C2128] border border-[#E8E9EA]/10 p-6 md:p-12 flex flex-col justify-between overflow-hidden transition-colors hover:border-[#FF2D55]/50"
                        >
                            <div className="absolute top-0 right-0 p-4 md:p-8 opacity-10 font-black text-[80px] md:text-[120px] leading-none select-none group-hover:text-[#FF2D55] transition-colors duration-500">
                                {step.id}
                            </div>

                            <div className="relative z-10">
                                <div className="w-12 h-1 md:w-16 md:h-1 bg-[#FF2D55] mb-6 md:mb-8" />
                                <h3 className="text-2xl md:text-4xl font-bold mb-4 md:mb-6 text-[#E8E9EA]">{step.title}</h3>
                                <p className="text-base md:text-xl text-[#E8E9EA]/60 leading-relaxed max-w-md">
                                    {step.description}
                                </p>
                            </div>
